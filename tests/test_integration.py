...
def mock_server():
    ...
    class MockServer:
        ...
        def get_response(self, method, url):
            ...
            endpoint = (
                url.split("localhost:3004")[-1] if "localhost:3004" in url else url
            )
            key = f"{method.upper()}:{endpoint}"
...
def client():
    ...
    return SeekSphereClient(
        {"base_url": "http://localhost:3004", "api_key": "test-org-id", "timeout": 30}
    )
...
class TestIntegration:
    ...
    def test_complete_workflow(self, mock_request, client, mock_server):
        ...
        mock_server.set_response(
            "/org/search_schema", "PUT", {"success": True, "message": "Schema updated"}
        )
        mock_server.set_response(
            "/org/tokens", "PUT", {"success": True, "message": "Tokens updated"}
        )
        mock_server.set_response(
            "/search",
            "POST",
