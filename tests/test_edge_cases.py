...
class TestEdgeCases:
    ...
    def test_base_url_variations(self):
        ...
        client1 = SeekSphereClient(
            {"base_url": "https://api.test.com/", "api_key": "test"}
        )
        ...
        client3 = SeekSphereClient(
            {"base_url": "https://api.test.com/v1/", "api_key": "test"}
        )
    ...
    def test_tokens_validation_edge_cases(self, client):
        ...
        with pytest.raises(
            ValidationError, match="All token values must be lists of strings"
        ):
            client.update_tokens({"tokens": {"category": ["token1", 123, "token2"]}})

        # Very large token structure
        large_tokens = {
            f"category_{i}": [f"token_{i}_{j}" for j in range(100)] for i in range(50)
        }
        try:
            with patch("requests.Session.request") as mock_request:
                mock_resp = Mock()
            ...
            pytest.fail(
                "Should not raise ValidationError for large but valid token structure"
            )
